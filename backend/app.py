...
from utils.file_reader import read_pdf, read_excel
...
#print("DEBUG: FILTERS_FILE path:", FILTERS_FILE, "Exists:", os.path.exists(FILTERS_FILE))
#print("DEBUG: LASTNAMES path:", LASTNAMES, "Exists:", os.path.exists(LASTNAMES))
...
def analyze_file():
    ...
    try:
        ...
        filter_mode = request.form.get("filter_mode", "names")  # default: names
        fuzzy_threshold = int(request.form.get("fuzzy_threshold", 85)) 

        # ✅ Επιλογή φίλτρων
        # --- Επιλογή φίλτρων: Custom έχει ΠΡΟΤΕΡΑΙΟΤΗΤΑ ---
        filter_file = request.files.get("filter_file")
        using_custom_filter = False

        if filter_file:
            # Ανέβηκε custom φίλτρο
            custom_path = os.path.join(app.config["UPLOAD_FOLDER"], filter_file.filename)
            filter_file.save(custom_path)
            #print(f"[analyze] Using CUSTOM filter: {custom_path}, Exists: {os.path.exists(custom_path)}")
            filters = load_filters(custom_path)
            using_custom_filter = True
            #print(f"[analyze] Using CUSTOM filter: {custom_path} ({len(filters)} lines)")
        else:
            # default συμπεριφορά με radio buttons
            if filter_mode == "plates":
                filters = load_filters(LASTNAMES)
            else:
                filters = load_filters(FILTERS_FILE)
        ...
        return jsonify({"results": results, 
                        ...
                        "using_custom_filter": using_custom_filter
        })
